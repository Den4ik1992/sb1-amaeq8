...
export function WeightSimulation({ initialParts = 10000, initialCalibration = 100 }: WeightSimulationProps) {
  ...
  const [simulationResults, setSimulationResults] = useState<{
    ...
    weightDistributionData: { x: number[]; y: number[] };
  } | null>(null);
  const [isSimulating, setIsSimulating] = useState(false);

  const generateWeightDistributionData = (parts: number[]) => {
    const binSize = distributionType === 'discrete' ? 0.05 : 0.02;
    const minWeight = Math.min(...parts);
    const maxWeight = Math.max(...parts);
    const bins: { [key: number]: number } = {};
    
    // Initialize all bins with 0
    for (let bin = Math.floor(minWeight / binSize) * binSize; 
         bin <= Math.ceil(maxWeight / binSize) * binSize; 
         bin += binSize) {
      bins[bin] = 0;
    }
    
    // Count occurrences
    for (let weight of parts) {
    ...
    return {
      x: sortedBins.map(([weight]) => Number(weight)),
      y: sortedBins.map(([_, count]) => count)
    };
  };
  ...
  const runSimulation = async () => {
    ...
    const weightDistributionData = generateWeightDistributionData(allParts);
              ...
              >
                <div className="bg-white rounded-xl shadow-lg p-6 h-[450px]">
                  <Plot
                    data={[
                      {
                        x: simulationResults.weightDistributionData.x,
                        y: simulationResults.weightDistributionData.y,
                        type: 'scatter',
                        mode: 'lines',
                        name: 'Gewichtsverteilung',
                        line: { 
                          color: '#3b82f6',
                          shape: 'spline',
                          smoothing: 1.3
